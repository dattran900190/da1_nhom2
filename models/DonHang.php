...
class DonHang
{
    ...
    public function addDonHang($tai_khoan_id, $ten_nguoi_nhan, $email_nguoi_nhan, $sdt_nguoi_nhan, $dia_chi_nguoi_nhan, $ghi_chu, $tong_tien, $phuong_thuc_thanh_toan_id, $ngay_dat, $ma_don_hang, $trang_thai_id)
    {
        try {
            ...
            return $this->conn->lastInsertId(); // lấy id để insert vào chi tiết DH
        } catch (Exception $e) {
    ...
    public function getChiTietDonHang($id){
        try {
            $sql = "SELECT * FROM chi_tiet_don_hangs WHERE id = :don_hang_id";
            $stmt = $this->conn->prepare($sql);
            $stmt->execute([
                ':don_hang_id' => $id
            ]);
    ...
    //     }
    // }
    public function addChiTietDonHang($don_hang_id, $san_pham_id, $don_gia, $so_luong, $kich_co, $thanh_tien)
{
    try {
        $sql = "INSERT INTO chi_tiet_don_hangs (don_hang_id, san_pham_id, don_gia, so_luong, kich_co, thanh_tien) 
                VALUES (:don_hang_id, :san_pham_id, :don_gia, :so_luong, :kich_co, :thanh_tien)";
        $stmt = $this->conn->prepare($sql);
    ...
    public function getAllPhuongThucThanhToanDonHang()
    {
        try {
